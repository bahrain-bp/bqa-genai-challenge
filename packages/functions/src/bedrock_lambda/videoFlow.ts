...
export const handler: APIGatewayProxyHandler = async (event: any, _context) => {
  try {
    ...
    const transferResponse = await axios.post(
      "https://u1oaj2omi2.execute-api.us-east-1.amazonaws.com/transferToGoogle",
    ...
    if (transferResponse.status !== 200) {
      throw new Error(
        `Error during transfer to Google: ${transferResponse.statusText}`
      );
    }
    ...
    const geminiResponse = await axios.post(
      "https://u1oaj2omi2.execute-api.us-east-1.amazonaws.com/gemini",
      JSON.stringify(geminiData)
    );

    if (geminiResponse.status !== 200) {
      throw new Error(
        `Error during Gemini API call: ${geminiResponse.statusText}`
      );
    }
