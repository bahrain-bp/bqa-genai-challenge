...
export const extractTextFromPDF = async (event: APIGatewayEvent, context: Context) => {
  try {
    ...
    const { bucketName, folderName = '', subfolderName = '', fileName } = requestBody;
    ...
    const startAnalysisResponse = await textractClient.send(startDocumentAnalysisCommand);
    console.log('Start Analysis Response:', startAnalysisResponse);
    ...
    return {
      statusCode: 200,
      body: JSON.stringify({ message: 'Text extracted successfully', text: extractedText }),
    };
  } catch (error) {
...
async function streamToBuffer(stream: NodeJS.ReadableStream): Promise<Buffer> {
  ...
  await pipelineAsync(stream, new Writable({ write(chunk, encoding, callback) { callback(); } }));
