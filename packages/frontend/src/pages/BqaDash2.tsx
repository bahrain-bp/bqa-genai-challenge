...
const BqaDash2 = () => {
  ...
  useEffect(() => {
    ...
    // Fetch initial data for the default standard
    fetchDataForStandard('Standard 1');
  }, [email]);

  const fetchDataForStandard = (standardName: string) => {
    // Example implementation - replace with your actual data fetching logic
    let newData: Package[] = [];
    if (standardName === 'Standard 1') {
      newData = [
        { name: 'File 1.ppt', invoiceDate: `Jan 13, 2023` },
        { name: 'File 2.png', invoiceDate: `Jan 13, 2023` },
      ];
    } else if (standardName === 'Standard 2') {
      newData = [
        { name: 'File 3.tsx', invoiceDate: `Jan 13, 2023` },
        { name: 'File 4.pdf', invoiceDate: `Jan 13, 2023` },
      ];
    }
    ...
    <DefaultLayout>
      ...
      <div className="flex justify-end py-4">
        ...
        <button className="px-5 py-2 bg-primary text-white rounded-md shadow-sm hover:bg-primary-dark focus:outline-none focus:ring-2 focus:ring-primary-dark focus:ring-opacity-50">
          <Link to={`/BqaRequestPage?email=${email}`}>Request Documents</Link>
        </button>
      </div>
